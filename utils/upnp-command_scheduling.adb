--------------------------------------------------------------------------------
--  This file is part UPnP package            				      --
--  									      --
...
with UPnP.Utils, UPnP.Controlers, UPnP.Services;

package body UPnP.Command_Scheduling is
   use SSDP, Ada.Strings.Fixed, Ada.Characters;
	    ...
	    when Discover | Alive | Bye_Bye =>
		 ...
		 with Value_Missing_After_Coma("space");
		  ...
		  when others => raise Parsing_Error
	       ...
	       --  UPnP.Utils.Pl_Debug("value: " & Command.Number'Img);
	       --  UPnP.Utils.Pl_Debug("reste: " & Commands_String(Next..Last));
   ...
   exception
	 ...
	 UPnP.Utils.Pl_Error("Parsing failed");
	 raise Parsing_Error;
   end Parse;
   ...
   procedure Batch(Device: in out Device_Type'Class;
	 ...
	 delay Amount_Of_Time;
      ...
      use UPnP.Controlers, UPnP.Services;
   begin
	 ...
	 case Schedule.Element(I).Command_Name is
	    ...
	    when Alive | Discover | Bye_Bye =>
		  ...
		  case Schedule.Element(I).Command_Name is
			...
			Notify_Alive(UPnP_Root_Device(Device), Header);

		     when Discover =>
			Pl_Debug("Discover" & N'Img & " /" &
				   Schedule.Element(I).Number'Img);
			M_Search(UPnP_Controler(Device), Header);

		     when Bye_Bye =>
			Pl_Debug("Bye bye" & N'Img & " /" &
				   Schedule.Element(I).Number'Img);
			Notify_Bye_Bye(UPnP_Root_Device(Device));
